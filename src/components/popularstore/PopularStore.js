...
function PopularStore(props) {
  ...
  useEffect(async () => {
    ...
    if (
      popularCompanyList.data &&
      popularCompanyList.data.content &&
      popularCompanyList.data.content.length
    ) {
      setCompanyList(popularCompanyList.data.content);
      globalDispatch({
        type: "ADD_POPULAR_COMPANIES",
        payload: popularCompanyList.data.content,
      });
    }
  }, []);
      ...
      companyList.map((item, ind) => {
        ...
        return (
            ...
            <Grid xl={2} lg={4}>
              ...
              >
                <Typography variant="h6" className={classes.couponavailable}>
                  {item.coupons_counter} <br /> Coupons Available
                </Typography>
